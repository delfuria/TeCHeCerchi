...
namespace TeCHeCerchi.iOS
{
	partial class WelcomeViewController : UIViewController
	{
		...
		{
			base.ViewDidLoad();
			NavigationController.NavigationBar.BarTintColor = TeCHeCerchi.Shared.Helpers.Color.Blue.ToUIColor();
		...
		private void GoToGame()
		{
			...
			var vc = storyboard.InstantiateViewController("QuestViewController") as UIViewController;
				...
				case WelcomeViewModel.GameLoadedPropertyName:
					{
						if (viewModel.GameLoaded)
						{
							if (viewModel.Game.Started)
								...
								ServiceContainer.Resolve<IMessageDialog>().SendMessage("TeCHe Cerchi", "La ricerca inizierà l'8 Gigno 2017. Torna presto a trovarci.");
						}
					}
					break;
			}
